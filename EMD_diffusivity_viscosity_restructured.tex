...
This section provides an overview of the checklist items for each property $(D$ and $\eta)$ and method (Green-Kubo and Einstein).
...
\section{General checklist items}

\subsection{General: Simulation set-up}

\subsubsection{General: Correct Ensemble}
...
Note that, although the best practice is to use the NVE ensemble (Steps 1-3), it is common to see values reported using the NPT (just Step 1) or NVT (Steps 1-2) ensemble. We strongly discourage the use of the NPT ensemble alone. The NVT ensemble has been used with success and is quite common. For example, Fanourgakis et al. reported that the NVT and NVE ensembles provide nearly identical results for viscosity \cite{Fanourgakis2012}. Therefore, either the NVT or NVE ensemble with NVE being preferred.
...
\subsection{General: Post-simulation analysis}
...
\subsection{General: Common pitfalls}
...
\section{Self-Diffusivity} \label{Self-Diffusivity}
...
\subsection{Equilibrium Green-Kubo Approach for Self-Diffusivity} \label{Self-Diffusivity Green-Kubo}
...
\subsection{Special topics for self-diffusivity}
...
\section{Viscosity} \label{Viscosity}
...
We recommend the Green-Kubo approach for predicting viscosity. This appears to be the most popular EMD method found in the literature. More importantly, less arbitrary data analysis methods exist that improve the reliability and reproducibility. Sections \ref{Viscosity Green-Kubo} and \ref{Viscosity Einstein} discuss checklist items specific to the Green-Kubo and Einstein approaches, respectively, for estimating viscosity.

\subsection{Checklist for Equilibrium Green-Kubo Approach for Viscosity} \label{Viscosity Green-Kubo}
...
\subsection{Equilibrium Einstein Approach for Viscosity} \label{Viscosity Einstein}
...
\subsection{Special topics for viscosity}

\cite{GROMACS} reports that viscosity ``is very dependent on the treatment of the electrostatics. Using a (short) cut-off results in large noise on the off-diagonal pressure elements, which can increase the calculated viscosity by an order of magnitude.'' \cite{Hess2002}
